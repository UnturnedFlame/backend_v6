...
def create_functions(U, num_second_level, data_path, time_key, fre_key, status_names):
    # 创建存储函数的字典
    data_all = feature_extraction.get_features(data_path, time_key, fre_key)
...
def create_functions_multiple_sensor(U, num_second_level, data_path, sensor1_key, sensor2_key, sensor3_key, status_names):
    # 创建存储函数的字典
    data_all = feature_extraction.get_features(data_path, sensor1_key, sensor2_key, sensor3_key)
    num_status = len(status_names)
    functions = {}
    # 循环创建函数B
    for j in range(num_status):
...
# 层次逻辑回归分析法
def model_pred(func_dict, test):
    X_sample = test
    feature_probabilities = []
    for i in range(X_sample.shape[1]):
        X_sample_feature = X_sample[:, [i]]
        probabilities = func_dict[i].predict_proba(X_sample_feature)

        # 将结果保存
        feature_probabilities.append(probabilities.flatten())
    feature_probabilities = np.stack(feature_probabilities, axis=0)
