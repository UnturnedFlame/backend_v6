...
from app1.module_management.algorithms.functions.preprocessing import bicubic_interpolation, polynomial_interpolation, \
    ...
    extract_features_with_multiple_sensors, wavelet_denoise_four_stages, dimensionless
...
class Reactor:
    ...
    def scaler(self, data_with_selected_features, multiple_sensor=False):
        ...
        scaled_data: pd.DataFrame = dimensionless(data, option=use_algorithm, multiple_sensor=multiple_sensor)

        self.module_configuration['无量纲化']['result']['raw_data'] = data
        self.module_configuration['无量纲化']['result']['scaled_data'] = scaled_data

        self.results_to_response['无量纲化']['raw_data'] = data.iloc[0].to_list()
        self.results_to_response['无量纲化']['scaled_data'] = scaled_data.iloc[0].to_list()
